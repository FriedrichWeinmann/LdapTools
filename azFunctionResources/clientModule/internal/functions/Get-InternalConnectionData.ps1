...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'LdapTools.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'LdapTools.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'LdapTools.Client.ProtectedToken'
