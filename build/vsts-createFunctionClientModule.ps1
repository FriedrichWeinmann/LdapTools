...
$parentModule = 'LdapTools'
if (-not $ModuleName) { $ModuleName = 'LdapTools.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\LdapTools'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\LdapTools.psd1"
$prereqHash = @{
